...
const Navbar = () => {
  ...
  return (
    <nav
      className={`fixed top-0 left-0 right-0 z-50 bg-white/95 backdrop-blur-lg border-b border-slate-200/60 transition-all duration-500 ${
        isVisible ? 'navbar-visible animate-slide-down' : 'navbar-hidden'
      }`}
    >
            ...
            <div className="flex flex-col space-y-1">
              {navLinks.map((link) => (
                <Link
                  ...
                  to={link.path}
                  onClick={closeMenu}
                  className={`flex items-center gap-3 px-4 py-3 rounded-lg font-medium text-base transition-all duration-300 ${
                    isActive(link.path)
                      ? 'text-slate-800 bg-slate-100'
                      : 'text-slate-600 hover:text-slate-800 hover:bg-slate-50'
                  }`}
